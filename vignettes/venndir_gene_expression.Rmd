...
A few features are immediately noticeable:
...
if (do_limma) {
  ...
  limmalist2g <- lapply(limmalist2, function(i){
    ...
    names(k) <- names(i);
    # use gene symbol
    igenes <- y$genes[names(k),"SYMBOL"];
    # remove entries with no gene symbol
    igenes <- igenes[nchar(igenes) > 0 && !is.na(igenes)];
    # choose the first entry per unique gene symbol
    j <- i[match(unique(igenes), igenes)];
    # assign name by gene symbol
    names(j) <- unique(igenes);
    j;
  })
...
partly because those two contrasts show very high concordance.
This experiment design is quite common, to have three groups,
and adding directionality for the third contrast can
be confusing at first. (See section below on interpreting
