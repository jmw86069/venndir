...
#'    the label layout. Passed as `pattern` to `sample_JamPolygon()`.
#'    * `"offset"` implements close to hexagonal/triangular arrangement,
#'    with adjustable xy ratio via argument `xyratio`. Every other point
#'    is "offset" upward by half the row height.
#'    * `"columns"` implements effectively the same approach with no
